...
const snap = await getDocs(collection(db, 'perguntas', 'leve', 'itens'));
...
const transformedPath = path.join(process.cwd(), 'backups', `leve-questions-translated-${stamp}.json`);
await fs.writeFile(
  transformedPath,
  JSON.stringify(
    {
...
for (let i = 0; i < translatedDocs.length; i += 400) {
  ...
  for (const question of slice) {
    const ref = doc(db, 'perguntas', 'leve', 'itens', question.id);
    batch.set(
      ref,
      {
